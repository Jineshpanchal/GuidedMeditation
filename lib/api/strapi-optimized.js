...
export const getMeditationPageData = async (slug) => {
  ...
  try {
    ...
    // Extract teacher IDs from meditation first
    ...
    const [relatedMeditations, teacherMeditations, allMeditations] = await Promise.all([
      ...
      // Get meditations by the same teacher(s)
      meditationTeacherIds.length > 0 ? getMeditations('card', {
        'filters[gm_rajyoga_teachers][id][$in]': meditationTeacherIds,
        'filters[id][$ne]': meditation.id,
        'populate[Media]': '*',
        'pagination[limit]': 8
      }) : Promise.resolve([]),
    ...
    console.log(`Found ${teacherMeditations.length} meditations by same teacher(s)`);
